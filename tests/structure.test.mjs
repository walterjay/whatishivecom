...
import { loadPage, siteFileExists, textOf } from './helpers.mjs';

const doc = loadPage();

test('has exactly one h1, and it names Hive', () => {
  const h1s = doc.querySelectorAll('h1');
  assert.equal(h1s.length, 1);
  assert.match(textOf(h1s[0]), /Hive/);
});

test('heading levels never skip (h1 → h2 → h3)', () => {
  let previous = 0;
...
test('local files referenced by the page exist', () => {
...
test('no render-blocking scripts, and no inline scripts other than JSON-LD', () => {
...
test('no inline style attributes (the Content-Security-Policy forbids them)', () => {
  assert.equal(doc.querySelectorAll('[style]').length, 0);
});

test('document basics: language, viewport, charset', () => {
